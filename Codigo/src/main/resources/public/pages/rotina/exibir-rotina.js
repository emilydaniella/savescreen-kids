...
function buscarCriancas(callback) {
    fetch(`http://localhost:8082/responsaveis/${userId}/criancas`)
        .then((response) => {
            if (!response.ok) {
        ...
        .catch((error) => {
            ...
            Swal.fire({
                title: 'Erro',
                text: 'Não foi possível carregar as crianças do responsável.',
                icon: 'error',
                confirmButtonColor: '#d6bdff',
            });
        });
...
ffetch(`http://localhost:8082/responsaveis/${userId}`)
    .then((response) => {
        if (!response.ok) {
